...
import { blue, green } from '@constant/theme';

const ConnectActionButtons: FC<{ phone: string }> = ({ phone }) => {
  return (
    <View style={{ flexDirection: 'row', gap: 10 }}>
      <Pressable onPress={() => Linking.openURL(`sms:${phone}`)}>
        <MaterialCommunityIcons name="email-outline" size={24} color={blue[5]} />
      </Pressable>

      <Pressable onPress={() => Linking.openURL(`tel:${phone}`)}>
        <MaterialCommunityIcons name="phone" size={24} color={green[5]} />
      </Pressable>
    </View>
  );
};
